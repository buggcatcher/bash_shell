...
// last_status è il valore di uscita dell’ultimo comando
// va passato come puntatore a tutte le funzioni che devono aggiornare o usare $?
// va aggiornato solo nel padre, dopo wait o dopo aver eseguito un builtin nel padre
int exe_exit(char **args, t_env **env, int last_status)
{
	...
	// TODO - free env, nodi, etc...
    (void)env;

	exit(exit_code);
	return (exit_code); // non dovrebbe succedere mai
}

/**
 * Versione speciale per gestire exit che ha bisogno del last_status.
 * Usata solo nel processo padre.
 */
int exec_builtin_with_exit(char **args, t_env **env, int last_status)
{
    if (!args || !args[0])
        return 1;
    else if (!strcmp(args[0], "exit"))
        return exe_exit(args, env, last_status);
    else
        return exec_builtin(args, env); // Delega agli altri builtin
}
