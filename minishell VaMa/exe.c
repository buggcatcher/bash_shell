...
char *join_path(char *dir, char *cmd)
{
    ...
    tmp = ft_strjoin_m(dir, "/");
    if (!tmp)
        return (NULL);
    full = ft_strjoin_m(tmp, cmd);
    free(tmp);
    return (full);
}
...
int exec_builtin(char **args, t_env **env)
{
    ...
    if (!args || !args[0])
        ...
        return exe_pwd(args, env);
	else if (!strcmp(args[0], "cd")) // controllare se is_child
        return exe_cd(args, env);
	else
		printf("%s", "non ancora implementato");
    return 1;
}
...
t_env *init_basic_env(void)
{
    ...
    if (path)
    {
        ...
        if (path_node)
        {
            path_node->key = ft_strdup_m("PATH");
            path_node->value = ft_strdup_m(path);
    ...
    if (home)
    {
        ...
        if (home_node)
        {
            home_node->key = ft_strdup_m("HOME");
            home_node->value = ft_strdup_m(home);
    ...
    if (pwd)
    {
        ...
        if (pwd_node)
        {
            pwd_node->key = ft_strdup_m("PWD");
            pwd_node->value = ft_strdup_m(pwd);
            pwd_node->exported = 1;
            pwd_node->next = env;
            env = pwd_node;
        }
