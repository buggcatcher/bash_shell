...
{
	if (dup2(from, to) == -1)					// dup2() può fallire: se from (oldfd EBADF) non è valido; se to (newfd EBADF) è fuori range;
	{								// se ci sono troppi file aperti (EMFILE), o restrizioni di sistema/sandbox (EPERM, EINVAL).					
		perror("dup2");
